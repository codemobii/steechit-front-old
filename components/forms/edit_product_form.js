...
export default function EditProductForm({ product, type }) {
  ...
  const [materialPrice, setMaterialPrice] = useState(
    type === "fabric"
      ? product.fabric.materialPrice !== undefined
        ? product.fabric.materialPrice
        : ""
      : product.tailor.materialPrice !== undefined
      ? product.tailor.materialPrice
      : ""
  );
  const [sowingPrice, setSowingPrice] = useState(product.tailor.sowingPrice);

  const [images, setImages] = useState(product.productPictures);
  ...
  const handleSubmit = async (evt) => {
    ...
    const data = {
      ...
      store: storeID,
      type: type,
      imageCategory: type === "fabric" ? "fabric" : "design",
      user: id,
      availableOptions: {
        measurement: null,
        price: price,
      ...
      tailor: {
        sowingPrice: parseFloat(sowingPrice),
        materialPrice: type === "tailor" ? parseFloat(materialPrice) : "",
      },
      fabric: {
        materialPrice: type === "fabric" ? parseFloat(materialPrice) : "",
      },
      tag: [],
    };
    console.log(data);
    try {
      const res = await axios({
        ...
        method: "PUT",
        url: `${process.env.apiUrl}products/${product._id}`,
        data,
      });
              ...
              >
                <Label d="block" m={{ b: "1rem" }}>
                  ...
                  <Input
                    placeholder="Price"
                    value={sowingPrice}
                    onChange={(e) => setSowingPrice(e.target.value)}
                  />
                </Label>
              </Col>
            </>
